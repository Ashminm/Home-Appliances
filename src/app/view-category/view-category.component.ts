...
export class ViewCategoryComponent implements OnInit {
  ...
  getViewCategory(clickedCategory?: string) {
    this.api.getAllProducts().subscribe(
      (res: any) => {
        if (res && res.length > 0) {
          // If a specific category is clicked, filter products based on it
          if (clickedCategory) {
            ...
            // If no category is clicked, display all products grouped by category
            res.forEach((product: any) => {
              const category = product.category;
              if (!this.itemsByCategory[category]) {
                this.itemsByCategory[category] = [];
                // Push category only if it's not already in the categories array
